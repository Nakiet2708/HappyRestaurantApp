...
export default function ProductDetails({ route }) {
  ...
  const toggleOption = (optionId) => {
    setSelectedOptions(prevState => {
      ...
      const newTotalPrice = Object.keys(newState).reduce((sum, key) => {
        if (newState[key]) {
  ...
  const addToCartHandler = () => {
    ...
    addToCart(cartItem);
    console.log(`Đã thêm ${quantity} ${product.name} vào giỏ hàng. Tổng: ${formatPrice(totalPrice * quantity)} VNĐ`);
    // Có thể thêm thông báo hoặc chuyển hướng đến giỏ hàng ở đây
  };
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView>
        ...
        <View style={styles.infoContainer}>
          ...
          <Text style={styles.totalPrice}>Tổng tiền: {formatPrice(totalPrice * quantity)} VNĐ</Text>
