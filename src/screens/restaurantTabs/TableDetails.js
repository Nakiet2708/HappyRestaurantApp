...
export default function TableDetail({ route }) {
  ...
  const [timeSlot, setTimeSlot] = useState('Buổi sáng');
            ...
            >
              <View style={styles.modalOverlay}>
                <View style={styles.modalContent}>
                  {timeSlots.map((slot) => (
                    <TouchableOpacity key={slot.value} onPress={() => selectTimeSlot(slot.value)}>
                      <Text style={styles.timeSlotOption}>{slot.label}</Text>
                    </TouchableOpacity>
                  ))}
